//! `linkport-cli` — terminal entry point (console binary) for debugging,
//! registration and dev-mode serving. The windowless main app is the
//! sibling `linkport.exe`.
...
enum Commands {
    ...
    /// (the OS-facing handler is the windowless linkport.exe).
    Open { url: String },
